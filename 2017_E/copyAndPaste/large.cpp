...
//    freopen("/Users/huangqingwei/Documents/C++ workspace/codeforces/input.txt", "r",stdin);
    freopen("/Users/huangqingwei/Downloads/A-large.in-2.txt", "r",stdin);
        ...
        for(int i=1;i<n;i++)
        {
            ...
            for(int j=1;j<=i;j++)
                for(int k=j;k<=i;k++)
                {
                    ...
                    candidate = min(candidate,dp[i+1][j][k]);
                }
            for(int jj=1;jj<=i+1;jj++)
                for(int kk=jj;kk<=i+1;kk++)
                    dp[i+1][jj][kk] = min(dp[i+1][jj][kk],candidate+1);
        }
        int ans = INF;
        rep(i,0,n)
            rep(j,0,n)
